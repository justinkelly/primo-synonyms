...
open (OUTPUT_CUSTOM, '>',"$primo_version/userSynonyms");
...
open (NAMES_FILE, '<', "$primo_version/userSynonyms")  or  die "Failed to read file : $! ";
my @not_sorted = <NAMES_FILE>;  # read entire file in the array
close (NAMES_FILE);
@sorted = sort { lc($a) cmp lc($b) } @not_sorted ;


open(FILE, '>',"$primo_version/userSynonyms" ) || die "unable to open file write <$!>";
